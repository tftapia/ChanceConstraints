...
def EconomicDispatch_LDT_old():
    ...
    omega_star = model.addMVar(1, vtype=GRB.CONTINUOUS, name="omega_star")     # Optimal critical value from the extreme event region
    omega_star_p = model.addMVar(1, vtype=GRB.CONTINUOUS, name="omega_star_p") # Auxiliar value that represent the positive value of "omega_star" used in a SOC
    omega_star_m = model.addMVar(1, vtype=GRB.CONTINUOUS, name="omega_star_m") # Auxiliar value that represent the negative value of "omega_star" used in a SOC

    lambda_n = model.addMVar(N, vtype=GRB.CONTINUOUS, name="lambda_n")     # Dual variable of the constrainst to obtain "omega_star"
    lambda_n_p = model.addMVar(N, vtype=GRB.CONTINUOUS, name="lambda_n_p") # Auxiliar value that represent the positive value of "lambda_n" used in a SOC
    lambda_n_m = model.addMVar(N, vtype=GRB.CONTINUOUS, name="lambda_n_m") # Auxiliar value that represent the negative value of "lambda_n" used in a SOC
    ...
    model.addConstrs(w_sigma**(-1)*omega_star - aux_lambda_ab_p[n] + aux_lambda_ab_m[n] == 0 for n in range(N))
    model.addConstrs(-p_n_max[n] + p_n[n] + aux_omega_ab_p[n] - aux_omega_ab_m[n]  == 0 for n in range(N))
